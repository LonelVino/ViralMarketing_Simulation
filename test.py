...
def get_node_color(G): 
    ...
    for node in G:
        ...
        print(color_dict, type(G))
        color_list.append(color_dict[G.nodes[node]["state"]])
    return color_list 


def visualization(Graph, position, SI_list, is_random=False):
    '''
    ...
    '''
     ...
     ### Plot the propagation network 
    fig1, ax1 = plt.subplots(figsize=(24, 16)) # set the size of the pic (24X16)
    ax1.axis("off") # shutoff the axis
    if is_random:
        ax1.set_title("Propagation Network with Random Selected Users")
    else:
        ax1.set_title("Propagation Network with Most Influential Users")        
    plt.box(False) # set picture without box(border)
    nx.draw(Graph, pos = position, node_size=1, width = 0.0001, node_color = get_node_color(Graph),ax=ax1, edge_color = "#D8D8D8")
...
def graph_draw(i,G,pos,ax,infeRate, ini_Infectors): 
    ...
    if i == 0: # In the first frame, plot the network
        nx.draw(G, pos = pos, node_size=1, width=0.0001, node_color = get_node_color(G), edge_color = "#D8D8D8", ax=ax)
    else: # In Other frames, update each nodes
        updateNetworkState(G, infeRate, ini_Infectors, visited)
        rep_node_color = ['white' if x=='blue' else x for x in get_node_color(G)]
        nx.draw_networkx_nodes(G, pos = pos, node_size=3, node_color = rep_node_color, node_shape='o', ax=ax) 
    plt.close()
...
if __name__ == "__main__":
    ...
    visualization(Graph_ini, position, SI_list)   
    visualization(Graph_rd, position, rd_SI_list, is_random=True)
    ...
    SI_list = []; rd_SI_list = []     # record number of infected users and non-infected users     
    # generate nimation of network 
    fig, ax = plt.subplots(figsize=(24, 16)) # set the size of the pic (24X16)
    ani = animation.FuncAnimation(fig, graph_draw, frames= range(0,days),fargs=(Graph_ini,position,ax,infeRate, ini_Infectors),  interval=1000)
    ani.save('assets/Propagation_animation_7d.gif', writer='imagemagick')
    rd_ani = animation.FuncAnimation(fig, graph_draw, frames= range(0,days),fargs=(Graph_rd,position,ax,infeRate, rd_Infectors),  interval=1000)
